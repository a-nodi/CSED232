...
void compute_inversioned_img(const Image<RGB8b>& img, Image<RGB8b>& inversioned_img){
	/*
	* The function that performs color inversion of given image
	* 
	* :param const Image<RGB8b>& img: The given image
	* :param Image<RGB8b>& inversion_img: The image result of given image's color inversion
	* 
	* :return void:
	*/

	inversioned_img = Image<RGB8b>(img.width(), img.height(), RGB8b(0, 0, 0));
	// for every channel in every pixel
	for (int y = 0; y < img.height(); y++) {
		for (int x = 0; x < img.width(); x++) {
			for (int ch = 0; ch < 3; ch++) {
				inversioned_img[y][x][ch] = 255 - img[y][x][ch]; // Color inversion
			}
		}
	}
}

void compute_alpha_blended_img(const Image<RGB8b>& img, Image<RGB8b>& alpha_blended_img, float alpha) {
	/*
	* The function that performs alpha blending of given image
	*
	* :param const Image<RGB8b>& img: The given image
	* :param Image<RGB8b>& inversion_img: The image result of given image's alpha blending
	* :param float alpha: alpha value(transparency)
	* :return void:
	*/

	alpha_blended_img = Image<RGB8b>(img.width(), img.height(), RGB8b(0, 0, 0));
	// for every channel in every pixel
	for (int y = 0; y < img.height(); y++) {
		for (int x = 0; x < img.width(); x++) {
			for (int ch = 0; ch < 3; ch++) {
				alpha_blended_img[y][x][ch] = int(img[y][x][ch] * alpha + 255 * (1 - alpha)); // Alpha blend image into white background
			}
		}
	}
}
...
/*
int main() {
	// Other main that performs color inversion and alpha blending
	Image<RGB8b> img;
	load_bmp("input.bmp", img);
	Image<RGB8b> inversioned_img;
	
	std::cout << "Perform color inversion" << std::endl;
	compute_inversioned_img(img, inversioned_img);
	save_bmp("inversioned_img.bmp", inversioned_img);
	
	
	std::cout << "Perform alpha blending with alpha value 0.5" << std::endl;
	Image<RGB8b> alpha_blended_img;
	compute_alpha_blended_img(img, alpha_blended_img, 0.5);
	save_bmp("alpha_blended_img.bmp", alpha_blended_img);
	
	// compute_alpha_blended_img(inversioned_img, alpha_blended_img, 0.5);
	// save_bmp("inversioned_alpha_blended_img1.bmp", alpha_blended_img);

	// compute_inversioned_img(alpha_blended_img, inversioned_img);
	// save_bmp("inversioned_alpha_bleneded_img2.bmp", inversioned_img);
}
*/